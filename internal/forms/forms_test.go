...
func TestForm_MinLength(t *testing.T) {
	...
	form.MinLength("x", 10)
	if form.Valid() {
		t.Error("form show minlength for non-existent field")
	}
	...
	if !form.Valid() {
		t.Error("shows minlength of 1 not met when it is")
	}
...
func TestForm_isEmail(t *testing.T) {
	...
	form.IsEmail("x")
	if form.Valid() {
		t.Error("form shows valid email for non-exient field")
	}
	...
	form.IsEmail("email")
	if !form.Valid() {
		t.Error("got an invalid email whn we should not have")
	}
