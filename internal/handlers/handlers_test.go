...
// data for the Reservation handler, /make-reservations route
var reservationTests = []struct {
...
}{
	{
		...
		expectedHTML:       `action="/make-reservations"`,
	},
	{
...
func TestPostReservation(t *testing.T) {
	for _, e := range postReservationTests {
		...
		if e.postedData != nil {
			req, _ = http.NewRequest("POST", "/make-reservations", strings.NewReader(e.postedData.Encode()))
		} else {
...
var chooseRoomTests = []struct {
...
}{
	{
		...
		url:                "/choose-room/1",
		expectedStatusCode: http.StatusSeeOther,
		expectedLocation:   "/make-reservations",
	},
	{
		name:               "reservation-not-in-session",
		reservation:        models.Reservation{},
